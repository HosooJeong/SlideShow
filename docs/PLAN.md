...
| 실사 포토북               | ✅      | I1: PBR 재질·방향광 그림자·환경광의 레이플랫 앨범 무대(`composeAlbum`, `AlbumView`), 잎 굽힘 셰이더, 표지 열림·페이지 넘김, 커피잔·낱장 인화지 소품                     |
| 포토북 CG 마감            | ✅      | I2: 넘김 대신 샷 리스트(3/4·플랫레이·디테일·그레이징·페이지 포커스)+컷·라이트리크, 창문 고보 스포트라이트 그림자, 리넨·종이 노멀맵, N8AO·샷별 DOF·블룸·색수차, 핸드헬드 |
| 펜 레이어                 | ✅      | J1·J2: 카메라 틸트 상한(30°), 마카 낙서 리본 셰이더·형광펜 곱하기 블렌딩·손글씨 리빌(나눔손글씨 펜), 낙서 아이콘 14종, 콜라주 보드 적용                                 |
| 펜 레이어 2               | ✅      | J3: 콜라주 패턴 배경 5종, 신문 편집자 빨간 펜(밑줄·동그라미·체크)+크라프트 책상판, 포토북 손글씨 캡션(스프레드 창 타이밍)                                               |
