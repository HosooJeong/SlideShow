...
type Slot = { id: string; x: number; y: number; w: number; h: number; rotation: number;
  frame: 'newsprint' | 'polaroid' | 'gilt' | 'none'; mediaId?: string;
...
5. **첫 무대**: 마법 신문(돌잔치판) 하나를 끝까지 완성도 있게 → 그 다음 앨범/액자 벽. 이 순서 동의하는지.
